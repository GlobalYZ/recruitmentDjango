...
'''类视图 一、继承视图django.views.generic.TemplateView，二，配置模板地址，三，配置URL。TemplateView是继承了三个类。
使用class改写视图实际上是面向对象改造的过程，Django内置的通用视图使代码更精简'''
class HomeView(TemplateView):
    template_name = 'job.html'# 可直接返回模板，无需render
def search(request):
    '''
...
def path_year(request,year):
    ...
    return HttpResponse("path_year视图函数")
def path_month(request,month):
    print(type(month))# <class 'int'>
    return HttpResponse("path_month视图函数: {}".format(month))
def index(request):
    '''
    模板语法：两种情况
    ① 变量    {{  }}
    ...
    user = "洋哥"# None
    date = time.time()
    img_url = '/media/images/樱木花道.jpeg' # 注意：这里的路径之所以是media而不是medias，是因为settings里设置过
...
def mysqlIndex(request):
    ...
    # 方式三 bulk_creat()批量新增数据
    # user1=User(name='a',password='1')
    # user2=User(name='b',password='2')
    # user_list=[user1,user2]
    # User.objects.bulk_create(user_list)
    # 方式四 get_or_create() 也算一个吧，有则返回，没有则创建记录
    ...
    '''==================================删除记录和修改记录===================================='''
    # delete() 调用者：model对象，QuerySet对象，单条和批量都可用，属于物理删除
    # ret = Book.objects.filter(price=100).first().delete()
    # count = Book.objects.filter(price=100).delete() # 返回的是 (2,{'app01.Book':2})，记录删除的个数，一般就是直接删
    # 使用save()修改单条数据，使用update()和bulk_update()批量修改数据
    # update() 调用者：QuerySet对象 count=?.objects.all(),update(*args,**kwargs) ，它不能修改外键关联的对象
    # count = Book.objects.filter(title="php").update(title="php_update")# 返回的是修改的个数


    # 1、all()方法，返回值是一个queryset对象，queryset是支持切片的，但只支持正切片，[obj1,obj2,obj3···]
    ...
    # 12、name__exact='张三' 等同于 name='张三'
    #     name__iexact='zhangsan' 像**值 name LIKE 'zhangsan'
    #     name__isnull=True/False 名称是否为空值
    #     name__contains='san'  包含**值，区分大小写
    #     name__icontains='san'  包含**值，不区分大小写
    #     price__gt=10,price__lt=200 __gt：> 的意思，# 大于等于是__gte，price__lt: < 的意思，小于等于是lte
    #     title__startswith="py" title中开头是"py"的，区分大小写，istartswith 大小写不敏感
    #     title__endswith="h" title中结尾"h"的，区分大小写的，iendswith大小写不敏感
    #     price__in=[100,200,300] price里有列表中这几个价格的可以过滤出来
    #     updated_at__date=datetime.date(2018,5,20) 日期查询，要传datetime对象进去
    #     register_date__range=['2018-01-30',datetime.date(2018,5,20)]) range：区间查询
    ...
    # <QuerySet [<Books: 红楼梦>, <Books: 三国演义>, <Books: 西游记>, <Books: 西游记>]>
    # 把所有的书籍价格都提高1块钱
    # ret = Books.objects.all().update(price=F("price")+1) # ret = 9
    # 把书名红楼梦并且价格是100的书籍查出来，下面这种写法是普通写法，"，"代表且的关系，但如果是"或"的话，就要用到Q了
    # ret = Books.objects.filter(title="红楼梦",price=100)
    ret_query  = Books.objects.filter(Q(title="红楼梦")|Q(price=101))# 语法上有"|"，"&"和~Q，代表"非"，如果同时使用键值对，加在Q后边
...
def query(request):
    ...
    # 单表分组查询 ORM语法：单表模型.objects.values("group by的字段").annotate(聚合函数("统计字段"))
...
def books_list_paginator(request):
    """ 分页-使用分页器 """
    page = request.GET.get('page', 1)
    book_list = Books.objects.all()
    # 第一步，取得分页器
    p = Paginator(book_list, 3)
    # 第二步，取得某一页的对象（实例）
    ...
    try:
        ...
        print('无效的页码')
        raise Http404
    return render(request, 'books_list_paginator.html', {
        'page_data': page_data
    })
class BooksListView(ListView):
    """ 分页处理, 面向对象 """
    # 对应的模板
    template_name = 'books_list_class.html'
    # 对应的ORM模型
    model = Books
    # 页面大小
    paginate_by = 3
...
@transaction.atomic()# 自动提交事务用以下这两种方法可以做到一致性，出现错误即回滚。（表中的主键ID会增加）
def user_signup_trans(request):
    """ 事务的使用-装饰器 """
    name = '13000000003'
    author = Author.objects.create(name=name,password='123456',nickname='王五')
    profile = AuthorDetail.objects.create(authordetail=author, usernamex=name)
    return HttpResponse('ok')
def user_signup_trans_with(request):
    ...
    with transaction.atomic():
        name = '13000000005'
        author = Author.objects.create(name=name,password='123456',nickname='王五')
        profile = AuthorDetail.objects.create(user=author, usernamex=name)
    return HttpResponse('ok')
...
def user_signup_trans_hand(request):
    ...
    try:
        author = Author.objects.create(username=username,password='123456',nickname='王五')
        profile = AuthorDetail.objects.create(user=author, username=username)
